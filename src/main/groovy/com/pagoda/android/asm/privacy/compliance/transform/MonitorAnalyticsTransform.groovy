...
import com.pagoda.android.asm.privacy.compliance.visitor.ClassVisitor
...
class MonitorAnalyticsTransform {
    private static HashSet<String> exclude = new HashSet<>(['android.support',
                                                            'androidx',
                                                            'com.google.android',
                                                            'com.bumptech.glide',
                                                            'com.tencent.tinker'])
    ...
    static File modifyClassFile(File dir, File classFile, File tempDir, ComplianceConfig monitorConfig) {
        File modified = null
        try {
    ...
    private static byte[] modifyClass(byte[] srcClass, ComplianceConfig monitorConfig) throws IOException {
        ClassWriter classWriter = new ClassWriter(ClassWriter.COMPUTE_MAXS)
        ClassVisitor classVisitor = new ClassVisitor(classWriter, monitorConfig)
        ClassReader cr = new ClassReader(srcClass)
        cr.accept(classVisitor, ClassReader.SKIP_FRAMES)
        return classWriter.toByteArray()
    }

    static File modifyJar(File jarFile, File tempDir, boolean nameHex, ComplianceConfig monitorConfig) {
        /**
         * 读取原 jar
         */
